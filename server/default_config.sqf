...
// General settings
A3W_startHour = 9;                // In-game hour at mission start (0 to 23)
...
A3W_globalVoiceWarnTimer = 30;      // Number of seconds for which global voice chat must be active before triggering a warning (0 = disabled)
...
A3W_spawnBeaconCooldown = 4*60;    // Number of seconds to wait between each use of an individual spawn beacon (0 = disabled)
A3W_townSpawnCooldown = 4*60;
A3W_spawnBeaconSpawnHeight = 3000; // Altitude in meters at which players will spawn when using spawn beacons (0 = ground/sea)
A3W_maxSpawnBeacons = 6;
...
A3W_cctvCameraSaving = 0;          // Save cctv cameras between restarts (0 = no, 1 = yes)
...
A3W_vehicleMaxUnusedTime = 0*24;   // Maximum parking time in hours after which unused saved vehicles will be marked for deletion (0 = no time limit)
...
A3W_buildingLootChances = 18;      // Chance percentage that loot will spawn at each spot in a building (0 to 100)
...
A3W_payrollAmount = 750;           // Amount of money rewa.rded per territory on each payroll
