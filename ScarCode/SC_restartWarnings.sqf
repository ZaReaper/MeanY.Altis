...
_restartWarningTxt = "(Proceed to a safe location) - Restart Incoming - Lock/force save objects/vehicles and log out at 5 minutes to ensure everything saves!"; // What the first line will say for all restart warnings
_warningSchedule = [20,15,10,5,2,1]; // At how many minutes should warnings be given
_enableDebug = false; // DEFAULT: false. Will show hintSilent with some info if true

/////// Configuration for DYNAMIC restarts
_restartInterval = 8; // Server uptime in hours | DEFAULT: 4 | MINIMAL: 0.5; (that is minimum of 30 minutes) | Ignore this setting if not using dynamic restarts
